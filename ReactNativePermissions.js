...
class ReactNativePermissions {
  constructor() {
    ...
    this.getPermissionTypes().forEach(type => {
      ...
      this[methodName] = p => {
        console.warn(
          `ReactNativePermissions: ${methodName} is depricated. Use getPermissionStatus('${type}') instead.`
        );
  ...
  getPermissionStatus(permission, type) {
    if (this.getPermissionTypes().indexOf(permission) >= 0) {
      ...
      return Promise.reject(
        `ReactNativePermissions: ${permission} is not a valid permission type on ${Platform.OS}`
      );
    }
  }
  ...
  requestPermission(permission, type) {
    ...
    if (Platform.OS === "android") {
      ...
      options = Math.floor(Math.random() * (maximum - minimum + 1)) + minimum;
  ...
  isOpenGps() {
    return RNPermissions.isOpenGps();
  }
}
